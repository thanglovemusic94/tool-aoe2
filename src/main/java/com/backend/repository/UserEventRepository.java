...
//            "from User u join UserEvent ue on u.id = ue.user.id where ue.event.id = :userEventId ")
    @Query(value = "SELECT ROW_NUMBER() OVER(ORDER BY ue.status_dong_tien desc) as STT,  u.nick_zalo as NickZalo, u.in_game as InGame, ue.status_dong_tien as StatusDongTien, ue.tien_dong as DaDong, ue.tien_ho_tro_giai as HoTroGiai " +
            "FROM users as u JOIN user_event as ue on u.id = ue.user_id WHERE ue.event_id = :userEventId ORDER BY ue.status_dong_tien desc, ue.created_date desc", nativeQuery = true)
